  ...
  <div class="selfStudy">
    <scroller lock-y :scrollbar-x=false ref="scroller">
      <div class="yy-tabs" ref="tabs" :style="{width:tWidth+'px'}">
        <div class="yy-tabItem" v-for="(item,idx) in navList" :key="idx" :class="{active:index===idx}"
             @click="index=idx">{{item}}
        </div>
      </div>
    </scroller>
    <div :style="{height:swiperHeight+'px',position:'relative'}">
      <swiper ref="swiper" v-model="index" style="width: 100%;" :height="swiperHeight+'px'" :show-dots="false"
              @on-index-change="onIdxChange">
            ...
            <ul v-if="item.isPreson===1" class="stu-list">
              <li v-for="(item1,idx) in dataList[index].childList" data-id="item1.chapter_id">
                <a href="javascript:void(0);">
                  <p class="photo" v-if="idx%2==0"><img
                    src="http://ued.t.huijiaoyun.com/tianyu_edu_dev/touch/yy_classroom/img/temp/selftaught_ico1.gif"
                    alt=""></p>
                  <p class="photo" v-else><img
                    src="http://ued.t.huijiaoyun.com/tianyu_edu_dev/touch/yy_classroom/img/temp/selftaught_ico2.gif"
                    alt=""></p>
                  <h4>{{item1.name}}</h4>
  ...
  export default {
    ...
    methods: {
      onIdxChange(idx) {
        ...
        let sW = screen.width;
        let left = tabs.children[idx].offsetLeft;
